...
public class ScaleCircleNavigator extends View implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener {
    ...
    public ScaleCircleNavigator(Context context, boolean vertical) {
        super(context);
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        switch (mode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                if (vertical) {
                    result = mMaxRadius * 2 + getPaddingLeft() + getPaddingRight();
                } else {
            ...
            case MeasureSpec.UNSPECIFIED:
                if (isVertical()) {
                    ...
                    if (mCircleCount <= 0) {
                        ...
                        result = (mCircleCount - 1) * mMinRadius * 2 + mMaxRadius * 2 + (mCircleCount - 1) * mCircleSpacing + getPaddingTop() + getPaddingBottom();
                    }
    ...
    protected void onDraw(Canvas canvas) {
        for (int i = 0, j = mCirclePoints.size(); i < j; i++) {
            ...
            if (isVertical()) {
                canvas.drawCircle(getWidth() / 2.0f, point.y, radius, mPaint);
    ...
    private void prepareCirclePoints() {
        ...
        if (mCircleCount > 0) {
            if (isVertical()) {
                ...
                int x = Math.round(getWidth() / 2.0f);
                int centerSpacing = mMinRadius * 2 + mCircleSpacing;
                int startY = mMaxRadius + getPaddingTop();
