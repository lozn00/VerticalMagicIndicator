...
public class FragmentContainerHelper {
    ...
    public static PositionData getImitativePositionData(List<PositionData> positionDataList, int index) {
        return getImitativePositionData(positionDataList, index, false);
    }

    public static PositionData getImitativePositionData(List<PositionData> positionDataList, int index, boolean vertical) {
        if (index >= 0 && index <= positionDataList.size() - 1) { // 越界后，返回假的PositionData
            ...
            if (vertical) {
                result.mTop = referenceData.mTop + offset * referenceData.height();
                result.mBottom = referenceData.mBottom + offset * referenceData.height();

                result.mLeft = referenceData.mLeft;
                result.mRight = referenceData.mRight;

                result.mContentTop = referenceData.mContentTop + offset * referenceData.height();
                result.mContentBottom = referenceData.mContentBottom+ offset * referenceData.height();


                result.mContentLeft = referenceData.mContentLeft;
                result.mContentRight = referenceData.mContentRight;
