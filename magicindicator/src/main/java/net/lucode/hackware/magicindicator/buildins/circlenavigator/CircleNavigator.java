...
public class CircleNavigator extends View implements IPagerNavigator {
    ...
    public CircleNavigator(Context context, boolean vertical) {
        super(context);
        this.vertical = vertical;
        init(context);
    }
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        switch (mode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                ...
                if (isVertical()) {
                    result = mRadius * 2 + mStrokeWidth * 2 + getPaddingLeft() + getPaddingRight();
                } else {
...
//                }
            ...
            case MeasureSpec.UNSPECIFIED:
                if (isVertical()) {
                    result = mTotalCount * mRadius * 2 + (mTotalCount - 1) * mCircleSpacing + getPaddingTop() + getPaddingBottom() + mStrokeWidth * 2;
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        if (mCirclePoints.size() > 0) {
            if (isVertical()) {
                canvas.drawCircle((int) (getWidth() / 2.0f + 0.5f), mIndicatorXOrY, mRadius, mPaint);
            } else {
                canvas.drawCircle(mIndicatorXOrY, (int) (getHeight() / 2.0f + 0.5f), mRadius, mPaint);
    ...
    private void prepareCirclePoints() {
        ...
        if (mTotalCount > 0) {
            ...
            if (vertical) {
                int x = (int) (getWidth() / 2.0f + 0.5f);
                int centerSpacing = mRadius * 2 + mCircleSpacing;
                int startY = mRadius + (int) (mStrokeWidth / 2.0f + 0.5f) + getPaddingTop();
                for (int i = 0; i < mTotalCount; i++) {
                    PointF pointF = new PointF(x, startY);
                    mCirclePoints.add(pointF);
                    startY += centerSpacing;
                }
                mIndicatorXOrY = mCirclePoints.get(mCurrentIndex).y;
            } else {
                ...
                mIndicatorXOrY = mCirclePoints.get(mCurrentIndex).x;
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if (mFollowTouch) {
            ...
            if (isVertical()) {
                ...
                mIndicatorXOrY = current.y + (next.y - current.y) * mStartInterpolator.getInterpolation(positionOffset);
            } else {

                mIndicatorXOrY = current.x + (next.x - current.x) * mStartInterpolator.getInterpolation(positionOffset);
            }
    ...
    public void onPageSelected(int position) {
        ...
        if (!mFollowTouch) {
            mIndicatorXOrY = isVertical() ? mCirclePoints.get(mCurrentIndex).y : mCirclePoints.get(mCurrentIndex).x;
            invalidate();
        }
    }
