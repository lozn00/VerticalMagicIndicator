...
public class CommonNavigator extends FrameLayout implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener {
    private ScrollViewI mScrollView;
    ...
    public CommonNavigator(@NonNull Context context, boolean verticalMode) {
        super(context);
    ...
    public void setVerticalMode(boolean _VerticalMode) {
        this._VerticalMode = _VerticalMode;
    }

    boolean _VerticalMode=false;
    ...
    public void init() {
        ...
        if (_VerticalMode) {
            if (mAdjustMode) {
                root = LayoutInflater.from(getContext()).inflate(R.layout.pager_navigator_layout_no_scroll_vertical, this);
            } else {
                root = LayoutInflater.from(getContext()).inflate(R.layout.pager_navigator_layout_vertical, this);
                mScrollView = (ScrollViewI) root.findViewById(R.id.scroll_view_vertical);   //如果同一个id,旋转屏幕的时候类型强转会出现问题，因为不是同一种类型控件。
            }
        } else {
            ...
            if (mAdjustMode) {
                ...
                mScrollView = (ScrollViewI) root.findViewById(R.id.scroll_view);   // mAdjustMode为true时，mScrollView为null
            }
        }
    ...
    private void initTitlesAndIndicator() {
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            ...
            if (v instanceof View) {
                ...
                if (isVerticalMode()) {
                    if (mAdjustMode) {
                        lp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,0);
    ...
    private void preparePositionData() {
        ...
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            ...
            if(mTitleContainer==null){
                throw new RuntimeException("need call init method");
    ...
    public void onSelected(int index, int totalCount) {
        ...
        if (!mAdjustMode && !mFollowTouch && mScrollView != null && mPositionDataList.size() > 0) {
            ...
            if (mEnablePivotScroll) {
                float scrollTo;
                if (isVerticalMode()) {
                    scrollTo = current.horizontalCenter() - mScrollView.getHeight() * mScrollPivotX;
                    if (mSmoothScroll) {
                ...
                // 如果当前项被部分遮挡，则滚动显示完全
                if(isVerticalMode()){
                    if (mScrollView.getScrollY() > current.mTop) {
                        if (mSmoothScroll) {
                            mScrollView.smoothScrollTo( 0,current.mTop);
                        } else {
                            mScrollView.scrollTo(0, current.mTop);
                        }
                    } else if (mScrollView.getScrollY() + getHeight() < current.mBottom) {
                        if (mSmoothScroll) {
                            mScrollView.smoothScrollTo(0,current.mBottom - getHeight());
                        } else {
                            mScrollView.scrollTo(0,current.mBottom - getHeight());
                        }
                    }
